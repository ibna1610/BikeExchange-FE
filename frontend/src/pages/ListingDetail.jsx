...
export default function ListingDetail() {
  ...
  const handleOrder = () => {
    if (!item) return
    // 👉 sau này gọi API createOrder tại đây
    alert(`Đặt mua xe: ${item.title}`)
  }
  ...
  const onTouchEnd = () => {
    if (!touchStart || !touchEnd || !images.length) return
            ...
            >
              {images.length > 0 ? (
                  <img
                      ...
                      alt={`${item.title} - Ảnh ${currentImageIndex + 1}`}
                  />
              ) : (
            ...
            <div className="detail-seller">
              ...
              <div>
                ...
                <p className="seller-contact">
                  {item.phone} • {item.contactAddress}
                </p>
              </div>
            </div>
